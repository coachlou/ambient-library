...
Drafts a new skill from work just completed, into `in-progress/` — the same
folder used for hand-built work in flight. This is how the library learns, but
a proposal is not a skill: it is unreachable by the router and cannot be
installed until `admin.md` promotes it into `library/`.

## CRITICAL: never write to in-progress without an explicit yes
...
covered this; want me to draft one into in-progress?" Silence or anything short of
...
2. If `in-progress/<name>/` already exists, **do not overwrite**. Append to
   its `PROPOSAL.md` Evidence section instead — a repeated proposal is the
   strongest signal it deserves promotion — and stop.
3. Write `in-progress/<name>/instructions.md`, same conventions as a real
...
- Write only under `in-progress/<name>/`. Nothing else in the repo changes.

## Rules

- Explicit yes before any write. Trace before imagination. Existing proposal →
  append evidence, don't overwrite.
- When done, tell the user it is in `in-progress/` and that "promote <name>" (in a clone
