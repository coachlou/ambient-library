...
Two modes. **Compact mode is the default** — run it for `/checkpoint`,
`/checkpoint:compact`, or any pre-compact/pre-clear save. Run **session mode**
only for `/checkpoint:session` or explicit check-in/check-out language on a
project with a `.context/` store.

---

# Mode 1: Compact (default)

Distill everything since the last checkpoint (or conversation start) into the
project's `.aai/checkpoint.md`, so a fresh chat — or this one after compaction —
can pick up with full context. Then hand off to `/compact`.

## Flow

**1. Locate or create the checkpoint file** at `<project-root>/.aai/checkpoint.md`
(the project root is the current working directory's repo root; plain cwd if
not a repo). Create the `.aai/` folder if absent — project-scoped context lives
there, per `~/.aai/README.md`. If a legacy `checkpoint.md` sits at the project
root, move it into `.aai/` unchanged first, so only one canonical copy exists.
If the file exists, read it — the "Current state" header and the most recent
dated section tell you what's already captured; don't repeat it.

**2. Distill the delta.** Review the conversation since the last checkpoint
(or since the last `/compact` or `/clear`, or the start). Capture
**conclusions, not justifications**: decisions made with a one-line why,
current state of the work, file paths touched, unresolved questions, exact
next steps. Skip anything re-derivable from the repo itself (code content,
git history) — reference it by path instead.

**3. Update the file** — two writes:
- **Rewrite the "Current state" header** at the top: 5–15 lines a stranger
  could read to resume immediately (objective, where things stand, next
  action, open questions).
- **Append a dated section** directly below the header (newest first) with
  the delta from step 2.
...
## Current state
<rewritten every checkpoint — objective, status, next action, open questions>

---

## <ISO timestamp> checkpoint

**Done since last:** <conclusions; decisions with a one-line why>
**Touched:** <files/paths>
**Open:** <unresolved questions, blockers>
**Next:** <exact resume point — file:line where applicable>
```

**4. Tell the user to compact.** You cannot run `/compact` yourself — it's a
user-level command. End with one line: *"`.aai/checkpoint.md` updated — run
`/compact` now."* (If they invoked this as a pre-`/clear` save, say `/clear`
instead. On harnesses without `/compact`, say "context ready to clear or
compact.")

## Gotchas

- **Conclusions, not transcripts.** The file is a handoff brief, not minutes
  of the conversation. If a section reads like a log, cut it down.
- **Never rewrite dated sections** — only the "Current state" header is
  mutable. The dated trail is the audit history.
- **A `.context/` store doesn't replace this.** If the project also uses
  session mode's store, still write `.aai/checkpoint.md` — conversation
  continuity and the multi-agent task board are different scopes.

---

# Mode 2: Session — project continuity protocol

This mode runs the check-in / check-out ritual for projects with a `.context/`
