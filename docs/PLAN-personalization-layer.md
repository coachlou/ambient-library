...
Status: **design draft v4.3, not built.** 2026-09-17. v1–v3.1 solved a narrower
...
- **Enablement needs no new router rule:** a `P/project.yaml` found by the walk
  ...
  `instructions.md` or `project.yaml` — an environment-only folder (notably
  `~/P/`) enables nothing.
...
### 5. Runtime — one block travels with each opted-in skill

Standalone installs never pass through `load.md`, so the body opens with this
(literal skill name; audit-checked). Single-project skills omit the bracketed
sentence.

> **Project.** `P` = `.aai/skills/gears-content/`. Start = a path the request
> gives; or a name = child of cwd, else sibling of the current project; else
> cwd. Named but not found: set-up intent → create it, ask the `project:` keys,
> write `P/project.yaml`; otherwise ask — never fall back to cwd. Project =
> nearest folder at/above start with `P/project.yaml` (stop below `~`). [None,
> and cwd has `P/environment.yaml` or `.ailib/gears-content/` → ask which child
> project, or a new one.] Else project = cwd: confirm (declined → stop), ask
> the keys, write `P/project.yaml`. Environment = nearest `P/environment.yaml`
> at/above the project, else `~/P`; save a new one in `P` of the nearest
> ancestor holding `.ailib/gears-content/` (never a project folder), else
> `~/P`. Files whole, never merged. Paths in `contract.yaml` (here) resolve
> under the project only. Every request re-resolves: before any write or send,
> re-read `project.yaml` from disk and state the project folder.

~190 tokens, paid only by opted-in skills. It is the whole runtime: no
resolver script, no registry, no stored pointer.
...
1. `load.md` line 13 counts any `.aai/skills/<name>/` as enabling. Tighten to
   "holds `instructions.md` or `project.yaml`" (§3). No ancestor-union of
...
3. `load.md` step 3's legacy cascade (`.ambient/`, `~/.aai/library/`,
   `overrides.md`) is unused and contradicts the fork rule. Retire (Lou's call).
...
1. **Shared rules (3 files), none of which alter a non-opted-in skill:**
   `.aai/skills/load.md` (line 13 tightening only; step 3's legacy cascade is
   left alone unless Lou approves retiring it — then it goes in the same edit),
   `.aai/instructions.md` (capability = first directory with
   `instructions.md`), `library/ambient-folder/instructions.md` (data-only
   project folders; the option documented).
2. **Runtime check:** a stub skill with a 5-line contract + two throwaway
   folders; a fresh subagent holding *only* the block runs: open by name from
   the parent; open from a nested path; New beside siblings; plain cwd with no
   layout; mid-session switch; a named folder that doesn't exist; a sibling
   named from inside a project; a stamped parent with zero projects; first
   environment save. The one runnable proof of the runtime — before
...
- No resolver script, project registry, stored active-project pointer, or
  "recent files."
...
- Retire `overrides.md` and the legacy cascade (unused; built in `6c871e1`)?
