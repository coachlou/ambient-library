...
The installer is a shell script: `${CLAUDE_SKILL_DIR}/library/context-mgr/install.sh`.

It takes one optional argument — the target directory (default: current working
directory):

```bash
bash "${CLAUDE_SKILL_DIR}/library/context-mgr/install.sh"           # install here
bash "${CLAUDE_SKILL_DIR}/library/context-mgr/install.sh" /path/to/project
```
...
2. **Check for conflicts.** Run the installer with `--check` first:
   ```bash
   bash "${CLAUDE_SKILL_DIR}/library/context-mgr/install.sh" --check <target>
   ```
...
3. **Install.** Run the installer for real:
   ```bash
   bash "${CLAUDE_SKILL_DIR}/library/context-mgr/install.sh" <target>
   ```
