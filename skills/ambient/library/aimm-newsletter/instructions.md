...
Then read `${CLAUDE_SKILL_DIR}/library/aimm-newsletter/assets/template.html` and replace `{{BODY_HTML}}` with the converted HTML.
...
- The template file path is absolute: `${CLAUDE_SKILL_DIR}/library/aimm-newsletter/assets/template.html` — use the Read tool to load it.
